...
export const getBuyer = async (req: Request, res: Response) => {
  try {
    ...
    const buyer = await Buyer.findOne(query).lean();
...
export const updateBuyer = async (req: Request, res: Response) => {
  try {
    ...
    const buyer = await Buyer.findOneAndUpdate(
      { _id: id, companyId: req.user.companyId, ownerId: req.user.userId },
      updates,
      { new: true }
    );
    if (!buyer) throw new AppError('Buyer not found', 404);
...
export const deleteBuyer = async (req: Request, res: Response) => {
  try {
    ...
    if (!req.user.companyId) throw new AppError('Company ID not found', 400);
    const { id } = req.params;
    const buyer = await Buyer.findOneAndDelete({ _id: id, companyId: req.user.companyId, ownerId: req.user.userId });
    if (!buyer) throw new AppError('Buyer not found', 404);
